...
import type { BookingTimeBlock } from '@/lib/db/queries/booking'
...
export function BlocksClient({ initial }: { initial: BookingTimeBlock[] }) {
  ...
  async function handleDelete(id: string) {
    ...
    setPending(id)
    try {
      const res = await fetch(`/api/magazyn/blocks/${id}`, { method: 'DELETE' })
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setSubmitting(true)
    try {
      const res = await fetch('/api/magazyn/blocks', {
        method: 'POST',
